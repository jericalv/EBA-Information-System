        ...
        <div class="card-body" style="padding:20px;">
            <form method="POST" action="{{ route('staff.concessionaires.update', $concessionaire->id) }}">
                ...
                <div style="display:grid;gap:8px;margin-bottom:12px;">
                    ...
                    <input type="text" name="business_name" value="{{ old('business_name', $concessionaire->business_name) }}" maxlength="255" required style="width:100%;padding:10px 12px;border:1px solid #cbd5e1;border-radius:8px;">
                </div>

                <div style="display:grid;gap:8px;margin-bottom:12px;">
                    <label style="font-size:13px;font-weight:700;color:#334155;">Description</label>
                    <textarea name="description" maxlength="2000" style="width:100%;padding:10px 12px;border:1px solid #cbd5e1;border-radius:8px;min-height:110px;">{{ old('description', $concessionaire->description) }}</textarea>
                </div>

                <div style="display:grid;gap:8px;margin-bottom:16px;">
                    <label style="font-size:13px;font-weight:700;color:#334155;">Location</label>
                    <input type="text" name="location" value="{{ old('location', $concessionaire->location) }}" maxlength="255" style="width:100%;padding:10px 12px;border:1px solid #cbd5e1;border-radius:8px;">
                </div>
